...
<script lang="ts" setup>
  ...
  import JnpfModeler from './selfDeveloped/modeler';
  ...
  function initBpmn() {
    ...
    const Modeler = props.type == 1 ? JnpfModeler : BpmnModeler;
    state.bpmn = markRaw(
      new Modeler({
        container,
        // 添加控制板
  ...
  function handleShapeAdded(element) {
    ...
    data.nodeName = getNodeName(data.type);
    ...
    if (currentData) {
      ...
      //全局属性开启签名后，新增的节点默认开启
      if (element.wnType == 'approver') data.hasSign = state.jnpfData.getValue('global', 'hasSign') || false;
