      ...
      <div class="condition-item-content">
        ...
        <a-row :gutter="8" v-for="(child, childIndex) in item.groups" :key="index + childIndex" wrap class="mb-10px">
          ...
          <a-col :span="7" class="mt-10px">
            <jnpf-select v-model:value="child.fieldValueType" :options="getConditionTypeOptions" @change="onFieldValueTypeChange(child)" />
          </a-col>
...
<script lang="ts" setup>
  ...
  const props = defineProps(['formConf', 'usedFormItems', 'updateJnpfData', 'formFieldsOptions', 'sourceIsStart']);
